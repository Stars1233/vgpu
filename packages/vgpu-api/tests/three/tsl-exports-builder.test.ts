...
test("combines disjoint WGSL modules from separate adapter calls", async () => {
...
test("forwards a function whose parameter has the same name as its call target", async () => {
...
test("forwards token-rich WGSL parameter types through Dawn", async () => {
...
test("forwards a template constant expression containing less-than", async () => {
...
test("forwards a parameter with comment trivia between its name and colon", async () => {
...
test("forwards a parameter with comment trivia after its colon", async () => {
  const { identity } = tslExports(
...
test("ignores comment trivia after a trailing parameter comma", async () => {
  const { identity } = tslExports(
...
test("contributes adapted WGSL to a Dawn render pipeline", async () => {
